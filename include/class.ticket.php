...
implements RestrictedAccess, Threadable, Searchable {
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    function onCreate(&$ticket) {
        ...
        if (!$ticket_id) {
            error_log("Ticket ID not found.");
            return;
        }
    
        // Connect to MySQL database
        $conn = mysqli_connect("localhost", "osticketss", "AllUsers@123", "osticketss");
        ...
        // Check the current staff_id (assigned agent)
    $check_staff_query = "SELECT staff_id FROM ost_ticket WHERE ticket_id = $ticket_id";
    $staff_result = mysqli_query($conn, $check_staff_query);
    $staff_row = mysqli_fetch_assoc($staff_result);

    $current_staff_id = $staff_row['staff_id']; // NULL if not assigned

    // Get the form entry ID linked to the ticket
    $query = "SELECT id FROM ost_form_entry WHERE object_id = $ticket_id LIMIT 1";
    $result = mysqli_query($conn, $query);
    $form_entry = mysqli_fetch_assoc($result);

    if (!$form_entry) {
        error_log("Error: No form entry found for ticket ID: $ticket_id");
        mysqli_close($conn);
        return;
    }
    ...
    // Get field IDs for 'District' and 'Plan'
    $district_field_query = "SELECT id FROM ost_form_field WHERE label = 'District' LIMIT 1";
    $district_field_result = mysqli_query($conn, $district_field_query);
    $district_field = mysqli_fetch_assoc($district_field_result)['id'];

    $plan_field_query = "SELECT id FROM ost_form_field WHERE label = 'Plan' LIMIT 1";
    $plan_field_result = mysqli_query($conn, $plan_field_query);
    $plan_field = mysqli_fetch_assoc($plan_field_result)['id'];

    // Fetch values for District and Plan
    $district_query = "SELECT value FROM ost_form_entry_values WHERE entry_id = $form_entry_id AND field_id = $district_field";
    $district_result = mysqli_query($conn, $district_query);
    $district = mysqli_fetch_assoc($district_result)['value'];

    $plan_query = "SELECT value FROM ost_form_entry_values WHERE entry_id = $form_entry_id AND field_id = $plan_field";
    $plan_result = mysqli_query($conn, $plan_query);
    $plan = mysqli_fetch_assoc($plan_result)['value'];

    // Default assignment: Bhaskar (ID: 6)
    $agent_id = 6;

    // If District = Pune and Plan = Advanced, assign to Sanjot (ID: 7)
    if ($district == "Pune" && $plan == "Advanced") {
        $agent_id = 7;
    }

    // If ticket is already assigned, don't change assignment
    if ($current_staff_id !== NULL) {
        error_log("Ticket ID: $ticket_id is already assigned to staff ID: $current_staff_id. No changes made.");
        mysqli_close($conn);
        return;
    }

    // Update ticket assignment
    $update_ticket = "UPDATE ost_ticket SET staff_id = $agent_id WHERE ticket_id = $ticket_id";
    if (!mysqli_query($conn, $update_ticket)) {
        error_log("Error updating ticket assignment: " . mysqli_error($conn));
    } else {
        error_log("Ticket assigned successfully to Agent ID: " . $agent_id);
    }
    ...
    //     error_log("DEBUG: onCreate function is triggered");
    
    //     $ticket_id = $ticket->getId();
    //     if (!$ticket_id) {
    //         error_log("ERROR: Ticket ID not found.");
    //         return;
    //     }
    
    //     error_log("DEBUG: Ticket ID: " . $ticket_id);
    // }
    /*
    ...
    static function create($vars, &$errors, $origin, $autorespond=true,
        ...
        // if (!$ticket->save()) {
        //     $errors['err'] = __('Unable to create the ticket.');
        //     return null;
        // }
